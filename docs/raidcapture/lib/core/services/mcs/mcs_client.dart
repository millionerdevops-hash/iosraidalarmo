...
class McsClient {
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      _socket!.listen(
        _onData,
        onError: (e) => debugPrint("MCS Socket Error: $e"),
        onDone: () => debugPrint("MCS Socket Closed"),
      );
      
    } catch (e) {
      debugPrint("MCS Connection Failed: $e");
    }
  }
  
  void _sendLogin() {
    debugPrint("Sending MCS Login...");
  ...
  void _handlePacket(int tag, List<int> body) {
    ...
    switch (tag) {
      case kLoginResponseTag:
        debugPrint("[MCS] Received LoginResponse!");
        break;
        
      case kHeartbeatPingTag:
        debugPrint("[MCS] Received Heartbeat Ping! Sending Ack...");
        _handleHeartbeatPing(body);
        break;
        
      case kHeartbeatAckTag:
        debugPrint("[MCS] Received Heartbeat Ack.");
        break;
        
      case kCloseTag:
        debugPrint("[MCS] Received Info/Close Packet.");
        break;
        ...
        try {
          ...
          if (msg.persistentId.isNotEmpty) {
            if (_persistentIds.contains(msg.persistentId)) {
               debugPrint("[MCS] 🛡️ Ignoring duplicate notification (PID: ${msg.persistentId})");
               return;
            }
          ...
          debugPrint("[MCS] DataMessage From: ${msg.from}, Category: ${msg.category}");
          _controller.add(msg);
        } catch (e) {
          debugPrint("[MCS] Failed to parse DataMessage: $e");
        }
        break;
        
      default:
        debugPrint("[MCS] Unknown Tag: $tag");
    }
  }
