...
class ConnectionManager {
  ...
  Future<void> connect() async {
    ...
    if (!wasSuccessful && !_serverInfo.useProxy) {
      debugPrint("[Connection] 🔄 Direct connection failed. Trying Facepunch Proxy Fallback...");
      wasSuccessful = await _tryConnect(true); // Force proxy 
      
      if (wasSuccessful) {
        debugPrint("[Connection] ✅ Proxy connection successful. Updating server preference.");
  ...
  Future<bool> _tryConnect(bool useProxy) async {
    ...
    debugPrint("[Connection] Attempting connection to $uri (Proxy: $useProxy)");
    ...
    try {
      ...
      _channel!.stream.listen(
        ...
        onError: (error) {
          debugPrint("[Connection] WebSocket Error: $error");
          if (!completer.isCompleted) completer.complete(false);
          disconnect();
        },
        onDone: () {
          debugPrint("[Connection] WebSocket Closed");
          if (!completer.isCompleted) completer.complete(false);
          disconnect();
        },
      );
      ...
      debugPrint("[Connection] Connection attempt failed: $e");
      return false;
    }
  }
  ...
  void _handleMessage(dynamic data) {
    try {
      ...
      debugPrint("[Connection] Error decoding message: $e");
    }
  }
  ...
  Future<AppMessage> setEntityValue(int entityId, bool value) async {
    ...
    if (_lastEntityRequests.containsKey(entityId)) {
      ...
      if (diff < _debounceDuration) {
        debugPrint("[Connection] 🛡️ Debouncing request for entity $entityId (Only ${diff.inMilliseconds}ms since last)");
