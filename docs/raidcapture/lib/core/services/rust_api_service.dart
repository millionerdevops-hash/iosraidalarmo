...
class RustApiService {
  ...
  Future<void> checkHistoryForPairing(String authToken) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("[RustAPI] History API Error: ${response.statusCode} ${response.body}");
      }
    } catch (e) {
      debugPrint("[RustAPI] Error fetching history: $e");
    }
  }
  ...
  Future<void> _saveServer(Map<String, dynamic> data) async {
    try {
      ...
      debugPrint("[RustAPI] History: Server processed $ip");
    } catch (e) {
      debugPrint("[RustAPI] Error saving server from history: $e");
    }
  }
  ...
  Future<void> _saveEntity(Map<String, dynamic> data) async {
     try {
       ...
       debugPrint("[RustAPI] History: Device processed ${name}");

     } catch(e) {
       debugPrint("[RustAPI] Error saving entity from history: $e");
     }
  }
}
