package com.raidalarm.fake_call.widgets
...
class RippleRelativeLayout : RelativeLayout {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        // Reposition ripple effects to center around avatar if it exists
        val avatarId = resources.getIdentifier("ivAvatar", "id", context.packageName)
        if (avatarId != 0) {
            val avatar = findViewById<View>(avatarId)
            if (avatar != null && avatar.visibility == View.VISIBLE) {
                val avatarCenterX = avatar.left + avatar.width / 2
                val avatarCenterY = avatar.top + avatar.height / 2
                
                for (rippleView in rippleViewList) {
                    val params = rippleView.layoutParams as LayoutParams
                    params.leftMargin = avatarCenterX - (rippleRadius).toInt()
                    params.topMargin = avatarCenterY - (rippleRadius).toInt()
                    rippleView.layoutParams = params
                }
            }
        }
    }
    ...
    override fun onAttachedToWindow() {
        ...
        // Start ripple animation after layout is ready
        post {
